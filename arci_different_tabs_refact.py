...
class different_tabs():
 ...
 #======================================================================================================
        #self.widgetFrame for temperature selection for D4
        #======================================================================================================
        ...
        # We are creating a container frame to hold all other widgets -- Tab2
        self.widgetFrame_temp = ttk.LabelFrame(self.tab_input, text="Choose D4 temp")
        self.widgetFrame_temp.grid(column=2, row=1, padx=8, pady=20)

        self.vars_temp = []
        self.picks_temp=['-40', '70', '20']
        
        for num,pick in enumerate(self.picks_temp):
            var = IntVar()
            chk = ttk.Checkbutton(self.widgetFrame_temp,  text=pick, variable=var)
            createToolTip(chk, 'Please select the temp for D4')
            chk.grid(column=num+1, row=0, sticky="W" )
            self.vars_temp.append(var)
                 
         
#       # Button for Analysis
#        self.buttonanalysis_temp = ttk.Button(self.widgetFrame_,text='Analyze',width=18, command=lambda:Thread(target=self.window.analysis_obj.analysis_alldays).start())
#        self.buttonanalysis_temp.grid(row=0, column=7)  
#        
        ...
        # Add some space around each label
        for child in self.widgetFrame_temp.winfo_children():
            child.grid_configure(padx=5, pady=5)
        ...
        information_frame = ttk.LabelFrame(self.tab_input, text="Choosing the external file")
...
#-------------------------------------------------       
    ...
    def update(self):
        ...
        self.states_checkbox_temp=list(map((lambda var: var.get()), self.vars_temp))
        
        print("The temperature states are:\n")
        for temp,state in zip(self.picks_temp,self.states_checkbox_temp):
            print(temp,state)
        
        self.ones_temp=[day for day,state in zip(self.picks_temp,self.states_checkbox_temp) if state == 1]
        self.zero_temp=[day for day,state in zip(self.picks_temp,self.states_checkbox_temp) if state == 0]
        
        print("The temperature selected are:\n")
        print(self.ones_temp)
            
        print("The temperature not selected are:\n")
        print(self.zero_temp)
        
    
        
        self.lst_indices_to_copy=[]
        for temp in self.zero_temp:
            if float(temp)==-40:
                self.lst_indices_to_copy.append(1)
            if float(temp)==70:
                self.lst_indices_to_copy.append(2)
            if float(temp)==70:
                self.lst_indices_to_copy.append(3)   
        print("List of indices to copy \n")
        print(self.lst_indices_to_copy)         
                
        self.lst_indices_present=[]
        for temp in self.ones_temp:
            if float(temp)==-40:
                self.lst_indices_present.append(1)
            if float(temp)==70:
                self.lst_indices_present.append(2)
            if float(temp)==70:
                self.lst_indices_present.append(3)         
        print("List of indices present \n")
        print(self.lst_indices_present)              
