# -*- coding: utf-8 -*-
"""
Created on Thu Jan 21 20:38:17 2021
...
def matlab_conver_func (a,dir_for_analy,sensor_name):
    ...
    df.iloc[10,3]=("{0:.2f}".format(np.mean(xsf))) #sf mean
    df.iloc[11,3]=("{0:.2f}".format(np.mean(Ysf))) #sftc mean
    df.iloc[12,3]=("{0:.2f}".format(avg_SF_HYSTERESIS_value))  #sf_hysterisis
    df.iloc[13,3]=("{0:.2f}".format(xsfdd[2]))   #SFDD
    df.iloc[14,3]=("{0:.2f}".format(SF_INRUN_STABILITY))  #SF_INRUN_S
    df.iloc[15,3]=("{0:.2f}".format(np.mean(xb)))#bias
    df.iloc[16,3]=("{0:.2f}".format( np.mean(yb)))#BTCF bias temperature coefficient
    df.iloc[17,3]=("{0:.2f}".format(avg_BIAS_HYSTERESIS_value))  #BIAS_HYS
    df.iloc[18,3]=("{0:.2f}".format(ybdd[2]))  #BiasDD Bias Day to Day Stability 	
    df.iloc[19,3]=("{0:.2f}".format(BIAS_INRUN_STABILITY))         #B_INRUN_S
